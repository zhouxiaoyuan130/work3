...
def build_system_prompt(platform_id: str, topic: str, other_platform: str) -> str:
    ...
    name = PLATFORM_INFO[platform_id]["name"]
    other_name = PLATFORM_INFO[other_platform]["name"]
...
def render_message(msg: Dict, autoplay_audio: bool = False):
    ...
    if role == "user":
        ...
        <div class="message platform{breakpoint_class}">
            <div class="message-avatar" style="background:{info['color']};color:white;">{info['icon']}</div>
            <div class="message-content">
                <div class="message-header">{info['name']}</div>
                {breakpoint_tag}
                <div class="message-bubble">{content}</div>
            </div>
        </div>
...
def render_summary():
    ...
    for pid in st.session_state.selected_platforms:
        ...
        st.progress(score / 100, text=f"{info['icon']} {info['name']}: {score}%")
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("⚙️ API 配置", expanded=False):
            # 尝试从 secrets 读取
            default_deepseek = st.secrets.get("DEEPSEEK_API_KEY", "") if hasattr(st, 'secrets') else ""
            default_zhipu = st.secrets.get("ZHIPU_API_KEY", "") if hasattr(st, 'secrets') else ""
            default_fish_key = st.secrets.get("FISH_AUDIO_API_KEY", "") if hasattr(st, 'secrets') else ""
            default_fish_voice = st.secrets.get("FISH_AUDIO_VOICE_ID", "") if hasattr(st, 'secrets') else ""
        ...
        if st.session_state.selected_platforms:
            names = [PLATFORM_INFO[p]["name"] for p in st.session_state.selected_platforms]
    ...
    p1, p2 = st.session_state.selected_platforms
    st.markdown(f"""
    ### {PLATFORM_INFO[p1]['icon']} {PLATFORM_INFO[p1]['name']} vs {PLATFORM_INFO[p2]['icon']} {PLATFORM_INFO[p2]['name']}
